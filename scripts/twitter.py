...
#get the own public ip
my_ip = urlopen('http://ip.42.pl/raw').read()
print my_ip

with open('hosts') as f:
    lines = f.read().splitlines()
total_host = len(lines)-1
print("total_host:",total_host)
#get the index of own ip in hosts file
for idx,line in enumerate(lines):
    if line == my_ip:
        own_idx = idx - 1
        break
print("own idx:",own_idx)

#generate the locations to catch twitter according to own public ip
location_region = []
for idx,location in enumerate(locations):
    if idx % total_host == own_idx:
        location_region += location
print("location region:",location_region)
    ...
    while True:
        try:
            for piece in keys:
                ...
                stream.filter(locations=location_region)
